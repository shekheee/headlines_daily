...
async function getArchiveStoryOnce(date = new Date()): Promise<ArchiveStory | null> {
  ...
  const data = await geminiJson<{
    ...
    `You are a meticulous history editor for an Indian audience. ${instruction}\n\n` +
      ...
      `- REELS PSYCHOLOGY for the "scenes": Beat 1 is THE HOOK — in ONE line make the subject unmistakable AND stop the scroll (a surprising fact or a real stake), naming the person/event. Then: set the scene, raise the tension, hit the turning point, and the FINAL beat lands the clear takeaway — the one thing to remember and why it still matters. Each line should make the viewer want the next one. Every sentence must be plain, concrete and understandable with sound OFF.\n` +
